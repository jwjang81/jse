...
public class CalendarDemo {
	public static void main(String[] args) {
		...
		// 클래스를 통해 바로 불러와서 인스터스를 생성한다.
		// 그래서 new Calendar() 와 같은
		// 생성자 문법을 사용하지 않고 아래처럼 생성했다.
		Calendar date = Calendar.getInstance();
		System.out.print("오늘 날짜 : ");  // printIn 은 줄 개행까지 포함이고, In 을 지우면 개행이 없다.
		 ...
		 * 자바 API 튜토리얼을 보면.. "calendars is JANUARY which is 0" 이라고 나와있다.
		 * 따라서 월은 0부터 시작하므로 위 식에서 1을 더해서 출력해야 한다.
		 * (date.get(Calendar.MONTH)+1) 괄호위치에 주의할것!!!
		 * */
		System.out.println();
		...
		/*
		 * 프로그램 트라이얼 보면 30일 사용제한같은 것이 걸린것을 본 케이스라면..
		...
		System.out.println("7일전의 시간을 얻고자 할때");
		date.add(Calendar.HOUR, -7); // 파라미터 에 마이너스 값을 주어서 처리하면 됨.
		System.out.println(toString(date)); // 스태틱이라 객체 생성없이 바로 가져다 씀.
		...
		/*
		 * 바로 출력을 하면 시스템에서 제공하는 시간들이 나온다.
		 * 따라서 toString() 을 오버라이딩 해야 한다.
		 * 여기서는 static 으로 별도로 작성한다.
		 * */
