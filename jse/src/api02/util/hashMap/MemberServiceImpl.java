 ...
 * 뜨는데, 마우스를 대시고, add, unimplement 를 클릭하세요.
 * 그럼 자동 오버라이딩이 됩니다.
 * */
public class MemberServiceImpl implements MemberService {
	...
	public void join(String id, String password, String name, int age,
		 ...
		 * valueOf() 는 api.lang.stringValueOfDemo 참조
		 * */
	...
	public void Login(String id, String password) {
		/*
		 * join() 에 지역변수로 map 을 선언했더니... *()는 메소드를 의미
		...
		if (!map.containsKey("id") || !(map.get("id").equals(id)) ) {
			...
			/*
			 * 에러유형 map.get("password")에서 "" 를 안쓰면
			 * 에러가 발생한다.
			 */ 
			 /*
			 * 3. 세번째 에러유형 NullPointException
			 * map.get(KEY) 에서 KEY는 맵을 선언한
