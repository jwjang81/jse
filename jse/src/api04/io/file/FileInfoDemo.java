...
public class FileInfoDemo {
	public static void main(String[] args) {
		...
		// file API 를 사용하고자 마음 먹었다면
		// 아무 의심없이 try{}catch(){} 를 쓰고 본다.
		try{
			BufferedReader in = new BufferedReader(new InputStreamReader(System.in));
			name = in.readLine();  // 스케너 처럼 한 단어가 아닌 한 문장을 읽어 들이기 위해
			// 스캐너보다 더 많은 문자를 읽어 들임으로 BufferedReader 사용을 권장함
		}catch(Exception ex){
			ex.printStackTrace();
			System.out.println("FileInfoDemo 에세 에러남 !!");
		}
		...
		if(file.exists()){
			...
			System.out.println("쓰기가늠여부 :" + file.canWrite());
