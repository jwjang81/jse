...
/*
 ...
 - 추상메소드를 사용하는 이유는 앞으로 추가되거나 구현되어야 하는 
          기능의 설계역활을 하며 실제 기능은 구현하지 않고
           메소드의 프로토타입만 구현하는 것을 말함.
 - 추상메소드는 건축물에서 구조를 이루는 철근과 같고 설계도롸 같은 역할을 함
 - 어떤 컴포너트나 클래스의 이용에 메소드를 공개할 목적으로 사용됨
 ...
 - 인터페이스에 선언된 메소드를 호출하면 그 인터페이스를 구현한 메소드가 
       	자동으로 호출됨
 - 인테페이스를 이용하면 인터페이스를 구현하는 클래스들은 메소드의 형태가 
 		같디 때문에 같은 메소드명을 이용하면서, 다양한 구현을 할 수 있음
 		이것을 마치 MySQL 용으로 개발된 프로그램을 Oracle 용으로 변경시에
 		JDBC 드라이버만 병경하면 프로그램이 작동하는 것과 같은 이치
 - 인테페이스는 기능을 구현해서 가지고 있는 것이 아니라
 		구현되어야 하는 구조를 가지고 있음.
...
/*
 ...
 - 각 클래스의 엠버변수에 접근하는 경우는 그 클래스 타입으로 형변환을 해주어야 함
 - 인터페이스에 변수를 선언한다면 주로 상수를 선언함
 - 메소드만 선언되는 경우가 대부분
 - 인터페이스 타입 객체 = new 인터페이스 구현클래스
           구현클래스  i = (구현클래스타입)인테페이스 객체
 * */
